...
@connect(
    ({dashboard}) => ({
        instances: dashboard.aliInstancesStatus['cn-qingdao']
    })
)
@withRouter
class AliQingDaoInstanceCount extends Component {
    componentWillMount() {
        this.props.dispatch(
            {
    ...
    render() {
        return (
            <ChartCard
                ...
                footer={<Field label="华北1(青岛)" />}
            />
        )
    }
}

export default AliQingDaoInstanceCount;
